...
extern "C" bool Port_LevelEditor_HandleKey(int sdlKey) {
...
extern "C" void Port_LevelEditor_OnRoomLoad(void) {
    ...
    if (hasCustomMap) {
        ...
        if (hasCustomRoomLayout) {
            ...
            if (input.good()) {
                // Print debug info for transition inspection
                std::fprintf(stderr, "[LEVEL EDITOR DEBUG] origin=(%d, %d), size=(%d, %d)\n", gRoomControls.origin_x, gRoomControls.origin_y, gRoomControls.width, gRoomControls.height);
                std::fprintf(stderr, "[LEVEL EDITOR DEBUG] Vanilla first 10 mapData values:\n");
                for (int i = 0; i < 10; i++) {
                    std::fprintf(stderr, "  0x%04X", gMapBottom.mapData[i]);
                }
                std::fprintf(stderr, "\n");

                // Determine the actual number of elements needed based on the room height.
                // Each row spans 64 elements. Overwriting only the rows needed by the room
                // preserves the second half of the buffers (the old room) during scrolling transitions.
                int numElements = (gRoomControls.height / 16) * 64;
                if (numElements > 4096) numElements = 4096;
                if (numElements < 0) numElements = 0;

                input.read(reinterpret_cast<char*>(gMapBottom.mapData), numElements * sizeof(unsigned short));
                input.seekg((4096 - numElements) * sizeof(unsigned short), std::ios::cur);
                input.read(reinterpret_cast<char*>(gMapTop.mapData), numElements * sizeof(unsigned short));

                // Align mapDataOriginal so that restore and gameplay systems operate properly
                std::memcpy(gMapBottom.mapDataOriginal, gMapBottom.mapData, numElements * sizeof(unsigned short));
                std::memcpy(gMapTop.mapDataOriginal, gMapTop.mapData, numElements * sizeof(unsigned short));

                std::fprintf(stderr, "[LEVEL EDITOR DEBUG] Loaded first 10 mapData values:\n");
                for (int i = 0; i < 10; i++) {
                    std::fprintf(stderr, "  0x%04X", gMapBottom.mapData[i]);
                }
                std::fprintf(stderr, "\n");
    ...
    std::fprintf(stderr, "[LEVEL EDITOR LOG] OnRoomLoad FINISHED successfully\n");
}
