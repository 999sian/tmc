/*
 * Native pickup keys for bundled `.logic` locations (rando_keymap.h).
 *
 * Picori generates seeds from bundled logic. Its location names must
 * resolve to the engine-native keys emitted by pickup hooks
 * (area<<16|room<<8|flag for ground items, or the scripted namespace below).
 * The numeric identities here come from the USA baserom and decompilation.
 *
 * The leading string matches each logic location name. Bindings are
 * applied after the logic loads and before a seed is generated. No logic loaded
 * means there are no names to bind (Rando_Keymap_Apply early-returns).
 *
...
typedef struct RandoKeymapEntry {
    ...
    uint8_t eu_jp_flag; /* nonzero only if the active EU/JP ROM uses a different flag */
} RandoKeymapEntry;
...
static const RandoKeymapEntry kLogicImportGroundKeys[] = {
    ...
    /* USA rec 0xD6EDC: Inn East 2F pot, Kinstone flag 0x27;
     * EU 0xD663B+0x8A4=rec+3. Its type2=2 copies the pot flag to the drop. */
    { "Town_Inn_Pot", 0x21, 0x09, 0x27 },
    /* USA rec 0xF5E48: GI HeartPiece flag 0xB4 (music house, EU-0xA44) */
    { "Town_MusicHouse_HP", 0x23, 0x05, 0xB4 },
    /* USA rec 0xF3674: Lon Lon Ranch pot with key flag 0x4D;
     * EU 0xF2C9B+0x9DC=rec+3. type2=2 preserves its pickup flag. */
    { "LonLon_RanchPot", 0x28, 0x02, 0x4D },
    ...
    /* USA rec 0xDD184: Blue L Kinstone, EU 0xDC8C3+0x8C4=rec+3 */
    { "Crenel_Melari_LowerMiddle_Dig", 0x10, 0x00, 0xB9 },
    { "Crenel_Melari_UpperTop_MiddleDig", 0x10, 0x00, 0xBA },
    /* USA rec 0xDD1A4: Blue S Kinstone, EU 0xDC8E3+0x8C4=rec+3 */
    { "Crenel_Melari_UpperTop_LeftDig", 0x10, 0x00, 0xBB },
    { "Crenel_Melari_UpperTop_RightDig", 0x10, 0x00, 0xBC },
    { "Crenel_Melari_UpperMiddle_RightDig", 0x10, 0x00, 0xBD },
    /* USA rec 0xDD1D4: Green G Kinstone, EU 0xDC913+0x8C4=rec+3 */
    { "Crenel_Melari_UpperMiddle_MiddleDig", 0x10, 0x00, 0xBE },
    { "Crenel_Melari_UpperMiddle_LeftDig", 0x10, 0x00, 0xBF },
    /* USA rec 0xDD1F4: Red W Kinstone, EU 0xDC933+0x8C4=rec+3 */
    { "Crenel_Melari_VeryBottom_Dig", 0x10, 0x00, 0xC0 },

    /* --- Castor Wilds underwater items (area 0x04 room 0) --------------- */
    /* USA recs 0xD9BF8..0xD9C18: Kinstone flags 0x20..0x22;
     * EU 0xD9347..0xD9367+0x8B4=rec+3. EU/JP entity list has two
     * extra hints before the items and uses flags 0x22..0x24. */
    { "Swamp_Underwater_Top", 0x04, 0x00, 0x20, 0x22 },
    { "Swamp_Underwater_Middle", 0x04, 0x00, 0x21, 0x23 },
    { "Swamp_Underwater_Bottom", 0x04, 0x00, 0x22, 0x24 },
    ...
    /* USA rec 0xF3C14: underwater Heart Piece, EU 0xF323B+0x9DC=rec+3 */
    { "Hylia_SunkenHP", 0x0B, 0x00, 0x09 },
...
#endif
        if (e->eu_jp_flag != 0 && (REGION_IS_EU || REGION_IS_JP))
            flag = e->eu_jp_flag;
